...
using Annotation = coreR5::Hl7.Fhir.Model.Annotation;
...
/// on top of <c>Hl7.Fhir.R5</c>. The path resolver handles only the
...
public sealed class R5Adapter : IFhirAdapter
{
    private static readonly Hl7.Fhir.Introspection.ModelInspector s_inspector =
        coreR5::Hl7.Fhir.Model.ModelInfo.ModelInspector;
    ...
    public object CreateResource(string typeName)
    {
        return typeName switch
        {
            ...
            _ => throw new ArgumentException(
                ...
                "Add a switch arm in R5Adapter or wait for Phase 6b to broaden coverage.",
                nameof(typeName)),
        };
    }
        ...
        [NotNullWhen(false)] out string? error)
    {
        ...
        if (resource is ResourceReference rr)
        {
            ...
            error = $"R5Adapter: unknown ResourceReference path '{path}'.";
            return false;
        }

        if (resource is not Observation obs)
        {
            error = $"R5Adapter: only Observation is supported in v0.x; got {resource?.GetType().Name ?? "null"}.";
            return false;
        }
        ...
        try
        {
            switch (p)
            {
                ...
                default:
                    ...
                    error = $"R5Adapter: unknown Observation path '{path}' (normalized '{p}').";
                    return false;
            }
        }
        catch (InvalidCastException ex)
        {
            error = $"R5Adapter: cannot coerce value for path '{path}': {ex.Message}";
            return false;
        }
    }
